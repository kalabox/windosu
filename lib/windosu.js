    ...
    var promise = function () {
            {
                return __when(Q.nfcall(fs.readFile, path.join(__dirname, '..', options.user ? 'windosu-runas.cmd' : 'windosu-elevate.cmd'), 'ascii'), function (__t0) {
                     ...
                     * inside of a binary
                     */
                    // Do weird things if we are inside the binary
                    var invis;
                    if (process.isPackaged || process.IsEmbedded) {
                        ...
                        if (fs.existsSync(confPath)) {
                            ...
                            invis = path.join(sysRoot, 'downloads', 'invisible.vbs');
                        }
                    }
                    // Otherwise do the normal
                    else {
                        invis = path.join(__dirname, '..', 'invisible.vbs');
                    }

                    console.log("GOT INVIS");
                    console.log(invis);
